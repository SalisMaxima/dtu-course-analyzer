...
const SCORE_COLUMNS = new Set(["avg", "avgp", "passpercent", "qualityscore", "workload", "lazyscore"]);

function createScore(value, key) {
  const badge = document.createElement("span");
  badge.className = "metric-value";
  badge.textContent = String(value) + (key === "avg" ? "" : "%");
  badge.style.backgroundColor = DTUAnalyzer.getMetricColor(value, key === "avg" ? 12 : 100);
  return badge;
}

// Keep keyboard focus on the same action when its label changes or rows rerender.
function rememberFocus(container) {
  const active = document.activeElement;
  const key = container.contains(active) ? active.dataset.focusKey : null;
  return () => {
    if (!key) return;
    const replacement = Array.from(container.querySelectorAll("[data-focus-key]"))
      .find((element) => element.dataset.focusKey === key);
    (replacement || document.getElementById("view-comparison")).focus({ preventScroll: true });
  };
}
...
function renderPage() {
  ...
  if (!tbody) return;
  const restoreFocus = rememberFocus(tbody);
  ...
  pageRows.forEach((row) => {
    ...
    COLUMNS.forEach(({ key, cssClass, numeric }) => {
      ...
      } else if (SCORE_COLUMNS.has(key)) {
        td.appendChild(createScore(row[key], key));
      } else {
    ...
    compareCell.className = "compare-column";
    const compareButton = document.createElement("button");
    const selected = state.selected.includes(row.course);
    compareButton.type = "button";
    tr.classList.toggle("selected", selected);
    compareButton.className = "dtu-btn compare-btn";
    compareButton.textContent = selected ? "Remove" : "Add";
    compareButton.disabled = !state.comparisonReady;
    compareButton.dataset.focusKey = row.course;
    compareButton.setAttribute("aria-label", `${selected ? "Remove" : "Add"} ${row.course} ${selected ? "from" : "to"} comparison`);
  ...
  if (!pageRows.length) {
    ...
    td.textContent = "No courses match your search. Try a course number or another name.";
    tr.appendChild(td);
    tbody.appendChild(tr);
  }
...
function appendComparisonRow(tbody, label, selectedRows, valueForRow) {
  ...
  labelCell.scope = "row";
  if (label === "Grade percentile" || label === "Grade participants") tr.className = "group-start";
...
function renderComparison() {
  ...
  const restoreFocus = rememberFocus(table);
  ...
  if (panel.hidden && panel.contains(document.activeElement)) document.getElementById("view-comparison").focus({ preventScroll: true });
  if (wasHidden && !panel.hidden) panel.open = true;
  document.getElementById("view-comparison").textContent = `Compare (${selectedRows.length}/${DTUAnalyzer.MAX_COMPARISONS})`;
  document.getElementById("selection-count").textContent = `${selectedRows.length} of ${DTUAnalyzer.MAX_COMPARISONS} selected`;
  document.getElementById("clear-comparison").disabled = selectedRows.length === 0;
  empty.hidden = selectedRows.length > 0;
  table.hidden = selectedRows.length === 0;
  table.style.minWidth = `${Math.max(660, 190 + selectedRows.length * 200)}px`;
  ...
  selectedRows.forEach((row) => {
    ...
    remove.type = "button";
    remove.className = "dtu-btn quiet comparison-remove";
    remove.textContent = "Remove";
    remove.dataset.focusKey = row.course;
    remove.setAttribute("aria-label", `Remove ${row.course} from comparison`);
    remove.addEventListener("click", () => toggleCourse(row.course));
    ...
    link.textContent = row.course;
    link.className = "comparison-course";
    th.appendChild(link);
  ...
  appendColoredComparisonRow(tbody, "Rating percentile", selectedRows, "qualityscore", "%", 100);
  appendColoredComparisonRow(tbody, "Workload percentile", selectedRows, "workload", "%", 100);
  appendColoredComparisonRow(tbody, "Lazy score percentile", selectedRows, "lazyscore", "%", 100);
...
async function initComparison() {
  ...
  const viewButton = document.getElementById("view-comparison");
  ...
  window.addEventListener("hashchange", () => {
    renderComparison();
    if (window.location.hash === "#compare") openComparison();
  });
  if (window.location.hash === "#compare") openComparison();
}
...
function initEvents() {
  document.querySelectorAll("#course-table th[data-key]").forEach((th) => {
  ...
  window.addEventListener("dtu-analyzer-language-changed", () => {
    if (state.sortKey === "name" || state.sortKey === "name_en") {
      state.sortKey = document.getElementById("course-table").classList.contains("lang-en") ? "name_en" : "name";
      update();
    }
    if (state.comparisonReady) renderComparison();
  });
...
async function initTable() {
  ...
  try {
    ...
    document.querySelector("#course-tbody td").textContent = "Course data could not be loaded. Try reloading the extension.";
    document.getElementById("course-table").setAttribute("aria-busy", "false");
    return;
  }
